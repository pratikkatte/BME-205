...
class Genome:
    ...
    def __init__(self, minK, maxK, cutoff, input_sequence):
        ...
        self.motif_counts = dict()
        self.expected_score = dict()
        self.genome_sequence = input_sequence
        self.genome_size = 0
        self.cutoff = float(cutoff)
        for seq in self.genome_sequence:
    ...
    def motifSearch(self):
        ...
        """
        for i in range(len(self.genome_sequence)):
            seq = self.genome_sequence[i]
    ...
    def motifScores(self):
        """
        Calls the function expectedScore(), zScore(), and calcProbab() to calculate exp expected_score and z-score for every motif that is stored in motif_counts. 
        
        :return - the function returns a dictionary (final_dict) which consists of all the motifs and its reverse component, expected score and z-score.
        """
    ...
    def display_scores(self, input_dict):
        """
        Displays all the motifs in decending orders and within every motif-size, sorts the z-score in ascending order. 
        """
...
if __name__ == "__main__":
    # inpfile = "Ecoli-UMN026.fa"
    # main(inFile=inpfile ,options=[ "--minMotif=3", "--maxMotif=8", "--cutoff=-4."])    
    main()
